module index(input [IMG_SIZE * DATA_WIDTH -1 : 0]  img,
   ...
   localparam img_index = y3 * IMG_MAX_X + x3;
   parameter result_index = 0;
   parameter kernel_index = 0;

   localparam y1 = result_index / IMG_MAX_X;
   localparam y2 = kernel_index / CONV_MAX_X;
   localparam y3 = y1 + y2 - 1;

   localparam x1 = result_index % IMG_MAX_X;
   localparam x2 = kernel_index % CONV_MAX_X;
   localparam x3 = x1 + x2 - 1;

   localparam is_inside = (y3 >= 0 && y3 < IMG_MAX_Y
			   &&
			   x3 >= 0 && x3 < IMG_MAX_X);

   localparam is_min_x = !x1;
   localparam is_max_x = x1 == IMG_MAX_X -1;
   localparam is_min_y = !y1;
   localparam is_max_y = y1 == IMG_MAX_Y -1;
   ...
   localparam is_in_border =
	      (is_min_y && (!is_min_x && !is_max_x))
	      || (is_max_y && (!is_min_x && !is_max_x))
	      || (is_min_x && (!is_min_y && !is_max_y))
	      || (is_max_x && (!is_min_y && !is_max_y));

   localparam PIXEL_END = (img_index+1)*DATA_WIDTH-1;
   localparam PIXEL_START = img_index*DATA_WIDTH;
   wire [31:0] p_img;

   localparam KERNEL_END = (kernel_index+1)*DATA_WIDTH-1;
   localparam KERNEL_START = kernel_index*DATA_WIDTH;

   localparam FIFO_START = ((result_index*CONV_SIZE)+kernel_index) * (DATA_WIDTH);
...
/*
	if (is_in_coin)
	  ...
	  ;//9
 */
	assign FIFO[FIFO_START +: DATA_WIDTH] = p_img * p_kernel;//passer uniquement p_imm ? rassembler les calculs ?
     end
