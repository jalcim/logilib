...
module cmp(a, b, eq, less, more);
   ...
   wire [WIRE-3:0]  out_ao;//, out_buf_and;
...
//   parallel_buf #(.WIRE(WIRE)) buf_and_inst(out_buf_and, out_nor);
   serial_and #(.WAY(WIRE), .WIRE(1)) and_inst(out_and, out_nor);//out_buf_and);

   nor nor_inst(eq, out_ao[6], out_nor);
   buf buf_inst_less(less, out_and);
   buf buf_inst_more(more, out_ao[6]);
endmodule
